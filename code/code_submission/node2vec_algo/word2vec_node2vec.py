...
class GraphEmbeddingwv:
    ...
    def training(self, walks, window_size, dimension, epochs, batch_words=32, sg=1, negative=5, learning_rate=0.025, min_count=0):
        ...
        for epoch in tqdm(range(epochs), desc="Training Word2Vec"):
            ...
            model.train(walks, total_examples=total_examples, epochs=1,compute_loss=True)
...
#         return prob_matrix
    ...
    def calculate_probability_matrix(self,random_walks):
        ...
        num_nodes = len(random_walks) // self.num_walks
        probabilities_matrix = np.zeros((num_nodes, num_nodes), dtype=float)

        for vi in range(1, num_nodes + 1):
            for vj in range(1, num_nodes + 1):
                occurrences = sum(
                    1 for walk in random_walks if walk[0] == str(vi) and str(vj) in walk[:self.walk_length]
                )
                total_walks = sum(1 for walk in random_walks if walk[0] == str(vi))
                probability = occurrences / total_walks if total_walks > 0 else 0
                probabilities_matrix[vi - 1][vj - 1] = probability
...
#         - min_count: Minimum word frequency for Word2Vec training.
#         """
#         callback_instance = LossPlotterCallback()
...
#         # Create a string for parameter names including window_size
#         param_names = f"window{window_size}_dim{dimension}_epochs{epochs}_batch{batch_words}_sg{sg}_neg{negative}_lr{learning_rate}_mn{min_count}"

#         # Plot training loss
#         callback_instance.plot_loss(param_names)

#         return model.wv



# class LossPlotterCallback(CallbackAny2Vec):
#     """
#     Callback to plot loss after each epoch
#     """
#     def __init__(self):
#         self.epoch = 0
#         self.loss_values = []
...
#     def plot_loss(self, param_names):
